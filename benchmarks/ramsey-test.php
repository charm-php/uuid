...
echo "UUID produced by ramsey/uuid: ".Ramsey\Uuid\Uuid::uuid1()."\n";
echo "UUID produced by charm/uuid:  ".Charm\Id::uuid1()."\n";

die();
echo "Comparing 100 000 UUID v3 for compatability with ramsey/uuid: \n";
for ($i = 0; $i < 100000; $i++) {
    $name = random_bytes(mt_rand(1, 30));
    $ruid = (string) Ramsey\Uuid\Uuid::uuid3($ns, $name);
    $cuid = (string) Charm\Id::uuid3($ns, $name);
...
for ($i = 0; $i < 100000; $i++) {
    $name = (string) mt_rand(0,999999999);
    $ruid = (string) Charm\Id::uuid3($ns, $name);
}
